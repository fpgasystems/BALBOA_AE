...
### CREED side: 
For running the RDMA-throughput experiments with a commercial RNIC on the remote side, the same bitstreams as for the CREED-CREED experiments can be used. However, one needs to compile the software contained in this experiment folder for an `inactive client`, which essentially connects to the RNIC and then spins in a busy loop responding to the RDMA-transactions of the RNIC. After the remote side has finished (i.e. printing of the test results), the CREED-side software can be terminated manually. 
