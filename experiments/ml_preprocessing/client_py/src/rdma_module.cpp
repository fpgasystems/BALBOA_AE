...
public:
    RDMAClient(const std::string& ip,
        ...
        if (enable_rdma) {
            // 原来的 RDMA 初始化
            mem_cpu = (int *) coyote_thread.initRDMA(max_size, coyote::DEF_PORT, server_ip.c_str());
            if (!mem_cpu) {
        ...
        mem_gpu = (int *) coyote_thread.getMem({coyote::CoyoteAllocType::GPU, max_size, false, DEFAULT_GPU_ID});
        if (!mem_gpu) {
